      ...
      <div className="grid gap-0 lg:grid-cols-5">
        <div className="space-y-5 p-5 sm:p-6 lg:col-span-3 lg:border-r lg:border-slate-200">
          <RangeSlider
            ...
            onChange={(v) => {
              const pct =
                inputs.homePrice > 0 ? inputs.downPayment / inputs.homePrice : 0.2;
              setInputs((prev) => ({
                ...prev,
                homePrice: v,
          ...
          <RangeSlider
            ...
            step={1000}
            format={(v) => `${formatCurrency(v)} (${formatPercent(downPct, 0)})`}
          />
          <RangeSlider
            label="Loan term"
        ...
        <div className="flex flex-col gap-4 bg-slate-50 p-5 sm:p-6 lg:col-span-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl border border-slate-200 bg-white p-3">
              <p className="text-xs font-medium text-slate-500">Intro payment</p>
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50/80 p-3">
              <p className="text-xs font-medium text-amber-800">Stress payment</p>
          ...
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex items-baseline justify-between gap-3 border-b border-slate-200/80 pb-2">
              ...
              <dd className="font-semibold tabular-nums text-slate-900">
                {formatPercent(Math.max(0, inputs.stressRate - inputs.introRate), 2)}
              </dd>
            </div>
          </dl>

          <p className="mt-auto text-xs leading-relaxed text-slate-500">
            Stress assumes the higher rate on today&apos;s balance with the full remaining
            term — a conservative budgeting check. Actual resets follow your margin, index,
            caps, and remaining schedule. Confirm caps on your Loan Estimate.
          </p>
        </div>
      </div>
    </section>

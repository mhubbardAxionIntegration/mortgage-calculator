...
export function FhaCalculator({
  ...
  const [inputs, setInputs] = useState(() => ({
    ...
    downPayment: Math.round(
      (loc.locationInputs.homePrice ?? BASE_DEFAULTS.homePrice) * 0.035,
    ),
  }));
  ...
  useEffect(() => {
    ...
    setInputs((prev) => ({
      ...prev,
      homePrice: price,
      downPayment: Math.round(price * 0.035),
      propertyTaxRate:
      ...
      >
        <div className="border-b border-slate-200 bg-slate-50 px-5 py-3 sm:px-6">
          <p className="text-sm font-medium text-slate-700">
            ...
            {loc.county
              ? ` · ${loc.county.name} County limit ${formatCurrency(loc.fhaLimit)} (${LOAN_LIMIT_YEAR})`
              : loc.state
                ? ` · ${loc.state.name} (select county for local limit)`
                : ""}
          </p>
        </div>
        ...
        <div className="grid gap-0 lg:grid-cols-5">
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm">
              ...
              <span>
                <span className="font-medium text-slate-800">
                  Finance upfront MIP into the loan
                </span>
                <span className="mt-0.5 block text-xs text-slate-500">
                  Most FHA borrowers roll UFMIP into the balance instead of
          ...
          <div className="flex flex-col gap-4 bg-slate-50 p-5 sm:p-6 lg:col-span-2">
            ...
            {overLimit && (
              ...
              >
                Financed loan ({formatCurrency(result.financedLoanAmount)})
                exceeds the {LOAN_LIMIT_YEAR} FHA 1-unit limit
                {loc.county ? ` for ${loc.county.name} County` : ""} (
                {formatCurrency(loc.fhaLimit)}). Lower the price, increase down
                payment, or choose a different county.
              </div>
            )}

            <dl className="space-y-3 text-sm">
              <ResultRow
                label={`FHA max loan (${LOAN_LIMIT_YEAR})`}
                value={formatCurrency(loc.fhaLimit)}
              />
              <ResultRow
              ...
              <ResultRow
                label="Upfront MIP"
                value={formatCurrency(result.upfrontMip)}
              />
              <ResultRow
                label={
                  inputs.financeUpfrontMip
                    ? "Financed loan amount"
                    : "Loan amount (UFMIP paid cash)"
                }
            ...
            <p className="mt-auto text-xs leading-relaxed text-slate-500">
              Loan limits are from HUD CHUMS {LOAN_LIMIT_YEAR} county files.
              MIP rates are illustrative — HUD tables and your FICO/LTV can
              change annual MIP.
            </p>
          </div>
        </div>

...
export function VaCalculator({
  ...
  useEffect(() => {
    setInputs((prev) => ({
      ...prev,
      homePrice: loc.locationInputs.homePrice ?? prev.homePrice,
      propertyTaxRate:
  ...
  const set = useCallback(
    <K extends keyof typeof BASE_DEFAULTS>(
      key: K,
      value: (typeof BASE_DEFAULTS)[K],
  ...
  const residualGuideline = loc.getVaResidual(inputs.familySize);
  const overPartialLimit =
    inputs.partialEntitlement &&
      ...
      >
        <div className="border-b border-slate-200 bg-slate-50 px-5 py-3 sm:px-6">
          <p className="text-sm font-medium text-slate-700">
            ...
            {loc.vaRegionLabel ? ` · residual region: ${loc.vaRegionLabel}` : ""}
          </p>
        </div>
        ...
        <div className="grid gap-0 lg:grid-cols-5">
          <div className="space-y-5 p-5 sm:p-6 lg:col-span-3 lg:border-r lg:border-slate-200">
            ...
            />
            <RangeSlider
              label="Family size (residual income)"
              ...
              format={(v) => `${v}`}
              hint={
                residualGuideline != null
                  ? `Guideline residual income ≈ ${formatCurrency(residualGuideline)}/mo (${loc.vaRegionLabel})`
                  : "Select a state for regional residual-income guidelines"
              }
            />

            <h3 className="pt-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
              VA funding fee & entitlement
            </h3>
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm">
              ...
              <span>
                ...
                <span className="mt-0.5 block text-xs text-slate-500">
                  Confirm exemption with your lender and Certificate of
                  Eligibility
                </span>
              </span>
            </label>
            <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm">
              <input
                type="checkbox"
                className="mt-0.5 h-4 w-4 rounded border-slate-300 text-sky-800 focus:ring-sky-600"
                checked={inputs.partialEntitlement}
                onChange={(e) => set("partialEntitlement", e.target.checked)}
              />
              <span>
                <span className="font-medium text-slate-800">
                  Limited / remaining entitlement
                </span>
                <span className="mt-0.5 block text-xs text-slate-500">
                  With full entitlement there is generally no VA loan cap;
                  remaining entitlement often tracks FHFA county conforming
                  limits ({formatCurrency(loc.conformingLimit)}
                  {loc.county ? ` in ${loc.county.name}` : ""}, {LOAN_LIMIT_YEAR}
                  )
                </span>
              </span>
            </label>
          ...
          <div className="flex flex-col gap-4 bg-slate-50 p-5 sm:p-6 lg:col-span-2">
            ...
            {overPartialLimit && (
              ...
              >
                Financed amount exceeds the {LOAN_LIMIT_YEAR} FHFA conforming
                limit ({formatCurrency(loc.conformingLimit)})
                {loc.county ? ` for ${loc.county.name} County` : ""}. With only
                remaining entitlement, lenders often cap guaranty against this
                limit — confirm with your COE.
              </div>
            )}
            ...
            <dl className="space-y-3 text-sm">
              {!inputs.partialEntitlement ? (
                <ResultRow
                  label="VA loan limit (full entitlement)"
                  value="No VA-imposed max"
                />
              ) : (
                <ResultRow
                  label={`FHFA conforming (${LOAN_LIMIT_YEAR})`}
                  value={formatCurrency(loc.conformingLimit)}
                />
              )}
              {residualGuideline != null && (
                <ResultRow
                  label={`Residual guide (${loc.vaRegionLabel})`}
                  value={`${formatCurrency(residualGuideline)}/mo`}
                />
              )}
              <ResultRow
            ...
            <p className="mt-auto text-xs leading-relaxed text-slate-500">
              Residual-income figures are approximate educational guidelines by
              region — not an underwriting decision. Funding-fee tiers and your
              COE control the real fee.
            </p>
          </div>
        </div>
      </section>

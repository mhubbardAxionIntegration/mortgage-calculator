...
export function RefinanceCalculator() {
  ...
  const set = useCallback(
    <K extends keyof typeof DEFAULTS>(key: K, value: number) =>
    ...
    >
      <div className="border-b border-slate-200 bg-slate-50 px-5 py-3 sm:px-6">
        <p className="text-sm font-medium text-slate-700">
          Refinance break-even — compare current vs new P&amp;I and recover closing costs
        </p>
      </div>
      ...
      <div className="grid gap-0 lg:grid-cols-5">
        <div className="space-y-5 p-5 sm:p-6 lg:col-span-3 lg:border-r lg:border-slate-200">
          ...
          <RangeSlider
            ...
            step={250}
            format={(v) => formatCurrency(v)}
            hint="Fees, points, title, appraisal — typically 2–5% of loan"
          />
          <RangeSlider
        ...
        <div className="flex flex-col gap-4 bg-slate-50 p-5 sm:p-6 lg:col-span-2">
          ...
          <dl className="space-y-3 text-sm">
            <ResultRow label="Current P&I" value={formatCurrency(result.currentMonthlyPI)} />
            <ResultRow label="New P&I" value={formatCurrency(result.newMonthlyPI)} />
            <ResultRow label="New loan amount" value={formatCurrency(result.newLoanAmount)} />
            <ResultRow
          ...
          <p className="mt-auto text-xs leading-relaxed text-slate-500">
            Estimates use principal &amp; interest only. Taxes, insurance, and MI may
            change when you refinance. Extending the term can cut the payment while
            increasing total interest — check both break-even and lifetime cost.
          </p>
        </div>
      </div>
    </section>

...
/** Intrinsic size of every hero source in `public/images/heroes` (3:2). */
export const HERO_INTRINSIC_SIZE = { width: 1536, height: 1024 } as const;

/**
 * Pre-built, statically-served mobile-width copy (828w, q68) of a hero —
 * generated by `scripts/optimize-heroes-mobile.mjs`. Served directly as a
 * plain `<img>` (no `/_next/image`) so phones never wait on a cold sharp
 * resize/re-encode for the LCP image.
 */
export function mobileHeroSrc(src: string): string {
  const idx = src.lastIndexOf("/");
  return `${src.slice(0, idx)}/mobile${src.slice(idx)}`;
}
